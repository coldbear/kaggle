...
dato <- fromJSON("C:\\Users\\ilias\\Desktop\\kaggle\\train.json")
...
x=unname(x)
j=1
feat=matrix(data=0,nrow=49352,ncol=1226)
featname=vector(mode="character",length=1226)
k=0
maxk=0
for (i in 1:length(x)){
  if (x[i]=="next"){j=j+1}
...
#print(sum(feat[,i]))}
sumfeat=0
colnames(feat)=featname
for (i in 1:1556){
  sumfeat[i]=sum(feat[,i])
}
feat=feat[,order(sumfeat, decreasing = TRUE)]
for (i in 1:27){
  dato=cbind(dato,feat[,i])
  colnames(dato)[length(dato)]=colnames(feat)[i]
}
#Manager ID
...
#Distance to subway
subway=read.csv("C:\\Users\\ilias\\Desktop\\kaggle\\NYC_Subway_Stations.csv")
mindistance=function(long, lat){
  i=0
  mindist=1000000
  dist=1000000
  for (i in 1:length(subway$longitude)){
    dist=((long-subway$longitude[i])^2+(lat-subway$latitude[i])^2)^0.5
...
dato$subdist=0
for (j in 1:length(dato$bathrooms))
{dato$subdist[j]=mindistance(dato$longitude[j],dato$latitude[j])}
#Address information omitted
dato$display_address[37184]=""
dato$address_differs=FALSE
for (i in 1:length(dato$bathrooms)){
  if (dato$display_address[i]==""){dato$address_differs[i]=TRUE}
  else { 
  x=unlist(strsplit(dato$street_address[i],"[^0-9]+",""))[1]
  y=unlist(strsplit(dato$display_address[i],"[^0-9]+",""))[1]
  if (x==""){l1=length(unlist(strsplit(dato$street_address[i],"[^0-9]+",""))[-1])}
  else (l1=length(unlist(strsplit(dato$street_address[i],"[^0-9]+",""))))
  if (y==""){l2=length(unlist(strsplit(dato$display_address[i],"[^0-9]+",""))[-1])}
  else (l2=length(unlist(strsplit(dato$display_address[i],"[^0-9]+",""))))
  if (l1>l2){dato$address_differs[i]=TRUE}
  }}
